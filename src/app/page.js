...
export default function Home() {
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Parallax for main video
      const mainVideo = document.querySelector('.main-video-container');
      if (mainVideo) {
        mainVideo.style.transform = `translateY(${scrollPosition * 0.3}px)`;
      }
      
      // Parallax for left column elements
      const leftColumn = document.querySelector('.left-column');
      if (leftColumn) {
        leftColumn.style.transform = `translateY(${scrollPosition * 0.1}px)`;
      }
      
      // Parallax for right column elements
      const rightColumn = document.querySelector('.right-column');
      if (rightColumn) {
        rightColumn.style.transform = `translateY(${scrollPosition * 0.2}px)`;
      }
    };
    ...
    setTimeout(() => {
      ...
      elements.forEach((el, index) => {
        ...
        el.style.opacity = '0';
        setTimeout(() => {
          el.style.transition = `transform 0.5s ease-out ${index * 0.1}s, opacity 0.5s ease-out ${index * 0.1}s`;
